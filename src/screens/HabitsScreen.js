...
export default function HabitsScreen({ onBack }) {
  ...
  const sections = useMemo(() => {
    const todaysHabits = todos.filter((t) => t.type !== 'once' && isDueOn(t, today));
    const groups = {};
    todaysHabits.forEach((t) => {
      const label = TYPE_LABELS[t.type] || t.type;
      if (!groups[label]) groups[label] = [];
      groups[label].push(t);
    });
    const order = Object.entries(TYPE_LABELS)
      .filter(([key]) => key !== 'once')
      .map(([, label]) => label);
    return order.filter((label) => groups[label]).map((label) => ({ title: label, data: groups[label] }));
  }, [todos, today]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {!loaded ? (
        ...
        <Text style={styles.empty}>No habits due today. Add one below.</Text>
      ) : (
        <SectionList
          sections={sections}
...
const styles = StyleSheet.create({
  ...
  overviewBtn: { backgroundColor: '#1F2937', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10, alignSelf: 'flex-start', marginTop: 10, marginBottom: 6 },
